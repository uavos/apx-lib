...
void Shiva::maneuver(void)
{
  //mode
  switch (var.mode) {
    case mode_EMG:      EMG(); break;
    case mode_RPV:      RPV(); break;
    case mode_UAV:      UAV(); break;
    case mode_WPT:      WPT(); break;
    case mode_HOME:     HOME(); break;
    case mode_STBY:     STBY(); break;
    case mode_TAXI:     TAXI(); break;
    case mode_TAKEOFF:  TAKEOFF(); break;
    case mode_LANDING:  LANDING(); break;
    default:
...
void Shiva::EMG(void)
{
  ...
  switch(apcfg.type){
    ...
    case type_helicopter:
      ...
      if(apcfg.type==type_helicopter) var.ctr_collective=var.rc_throttle*2.0-1.0;
      break;
    case type_blimp:
...
void Shiva::UAV(void)
{
  if((!var.stage)||(var.stage>2)) {     //reset cmd_NED
    var.stage=1;
    setNE(var.pos_NE);
    if(apcfg.type==type_helicopter) var.cmode|=cmode_hover;
  }else var.calc();//calc distHome
  navigate();
  calcETA(var.dHome);
  if(var.stage<=1){      //auto throttle if stage changed
...
void Shiva::WPT(void)
{
  ...
  bool bReached=false;
  switch(var.mtype) {
    case mtype_hdg: bReached=flyto(var.cmd_NE); break;
    case mtype_line:
...
void Shiva::HOME(void)
{
  switch(apcfg.type){
    ...
    case type_blimp:
      if(flyto(0,0)){
        if(var.cmd_airspeed!=0) var.cmd_airspeed=0;
      }else{
        if(var.cmd_airspeed==0) var.cmd_airspeed=apcfg.spd_cruise/2.0;
      }
      navigate();
      break;
    case type_boat:
...
void Shiva::STBY(void)
{
  ...
  if(var.stage==LANDING_stage::Cancel){
    ...
    if(var.loops==0)var.loops=1;
    if(mission.current.turnR==0)mission.current.turnR=apcfg.turnR;
    ...
    _var_float valt=(mission.current.pi&&(mission.current.pi->hmsl!=0))?mission.current.pi->hmsl:var.home_pos[2];
  ...
  if(apcfg.type==type_helicopter){
    ...
    if(var.rc_roll>dw)adj[1]=var.rc_roll-dw;
    else if(var.rc_roll<-dw)adj[1]=var.rc_roll+dw;
    if(var.rc_pitch>dw)adj[0]=-var.rc_pitch+dw;
    else if(var.rc_pitch<-dw)adj[0]=-var.rc_pitch-dw;
    if(!adj.isNull()){
    ...
    //thermals
    if(var.turnR==0 && (!mission.current.pi) && apcfg.type==type_airplane && ctr_Thermal()){
      ctr_Altitude();
      ctr_Airspeed();
      ctr_Thr();
      ctr_Governor();
  ...
  if(var.dWPT>(var.turnR*3.0)){
    ...
    uint xt=(t-time_VE)/1000;
    if(mission.current.timeS>xt) var.ETA=mission.current.timeS-xt;
    else var.ETA=0;
  }else{ //loops
    _var_float dist=fabs(var.turnR)*(2.0*PI);
    var.dWPT=dist*var.loops+dist*(360.0-(fabs(var.boundAngle(cSpan-var.wpHDG))+turnsVE*90.0))/360.0;
...
void Shiva::TAXI(void)
{
  ...
  switch(var.stage){
    ...
    case 4:{
      ...
      if(var.delta>(apcfg.taxi_turnR>0?apcfg.taxi_turnR:3))break;
      var.twidx++;
      prev_twidx=var.twidx;
      var.stage--;
    }break;
...
void Shiva::TAKEOFF(void)
{
  if(prev_rwidx!=var.rwidx) { //protect runway change
    if(var.stage<TAKEOFF_stage::Start)var.stage=0;
  ...
  uint stage=var.stage;
  switch(stage){
    case TAKEOFF_stage::init:{
      stage++;
      if(!apcfg.init_ignition)var.power|=power_ignition;
      var.ctrb&=~ctrb_ers;
      var.ctrb&=~ctrb_rel;
      var.power&=~power_agl;
      var.sw|=sw_taxi;
    ...
    case TAKEOFF_stage::WaitFP_do:
      if(mission.rw(var.rwidx)) stage=0;
      break;
    ...
    case TAKEOFF_stage::WaitPOS_do:
      if(var.status&status_gps) stage=0;
      break;
  ...
  uint stage_s=stage;
  switch(apcfg.type){
    case type_airplane:        stage=TAKEOFF_fixedwing(var.stage); break;
    case type_helicopter:       stage=TAKEOFF_helicopter(var.stage); break;
    case type_blimp:            stage=TAKEOFF_blimp(var.stage); break;
    case type_boat:             stage=TAKEOFF_boat(var.stage); break;
  }
  if(var.stage==stage)var.stage=stage_s;
  else var.stage=stage;
}
//==============================================================================
uint Shiva::TAKEOFF_fixedwing(uint stage)
{
  switch(stage){
    case TAKEOFF_stage::init:{
      var.mtype=mtype_line;
    ...
    case TAKEOFF_stage::Start:{
      ...
      set_roll(0);
      set_pitch(0);
      if(apcfg.to_type==to_type_taxitest) dmsg("Takeoff: Taxi test\n");
      else if(apcfg.to_type==to_type_launcher) dmsg("Takeoff: Launcher engaged\n");
      else dmsg("Takeoff: Start\n");
    }break;
    case TAKEOFF_stage::Start_do:{
      ...
      if(apcfg.to_type==to_type_taxitest) {
        ...
        if(apcfg.to_thrDelay<0) doThrStart=true;
        if(var.acc[0]>=((apcfg.to_launcherAx==0)?(9.81*1.5):apcfg.to_launcherAx))
          stage++;
      } else {
        doThrStart=true;
        if(var.ctr_throttle>=1)stage++;
      }
      if(doThrStart){
        //increase throttle slowly
        if(var.ctr_throttle<1){
          if(apcfg.to_thrRise>0) set_thr(var.ctr_throttle+apcfg.to_thrRise*dt);
          else set_thr(1);
        }
      }
    }break;
    case TAKEOFF_stage::Run:{
      if(apcfg.to_type==to_type_launcher) {
        if(apcfg.to_launcherDelay>0){
          time_s=t;
      } else stage=TAKEOFF_stage::Liftoff;
    ...
    case TAKEOFF_stage::Run_do:{
      if(apcfg.to_type==to_type_launcher) { //enters only if to_launcherDelay>0
        ...
        set_ail(0);
        set_elv(0,false);
        if((t-time_s)>=(apcfg.to_launcherDelay*1000)) stage++;
      } else {
        flytoLine(rwNE);
        set_roll(0,false);
        set_pitch(pitch_s);
        ...
        ctr_Taxi();
        ctr_Yaw();
        if(var.airspeed>apcfg.to_liftSpeed) stage++;
      }
      if(apcfg.to_thrDelay<=0) set_thr(1);
    }break;
    case TAKEOFF_stage::Liftoff:{
      var.cmd_airspeed=(apcfg.spd_VY==0)?apcfg.spd_cruise:apcfg.spd_VY;
      reg_Course.reset();
      pitch_s=apcfg.to_liftPitch*0.3;      // minimum for liftoff pitch
      if(apcfg.to_liftAGL)var.power|=power_agl;
      else var.power&=~power_agl;
      time_s=time_thr_s=t;
      if(apcfg.flaps_takeoff>var.ctr_flaps)var.ctr_flaps=apcfg.flaps_takeoff;
    }break;
    case TAKEOFF_stage::Liftoff_do:{
      if(apcfg.to_type==to_type_launcher){
        var.calc();
      ...
      stabilize();
      if(apcfg.flaps_takeoff<=0)ctr_autoflaps();
      //check if done
      if(!( ((var.altitude>(apcfg.to_liftAlt*0.5))&&(var.airspeed>var.cmd_airspeed)) ||
        (var.altitude>apcfg.altitude) )) time_s=t;
      else if((t-time_s)>200)stage++;
      if(apcfg.to_type==to_type_launcher){
    ...
    case TAKEOFF_stage::Climbing_do:{
      if(apcfg.to_type==to_type_launcher) var.calc();
      else flytoLine(rwNE);
      var.cmd_course=var.tgHDG;
      navigate();
...
//==============================================================================
uint Shiva::TAKEOFF_helicopter(uint stage)
{
  switch(stage){
    case TAKEOFF_stage::init:{
      var.cmode|=cmode_hover;
    ...
    case TAKEOFF_stage::Start_do:{
      ...
      var.cmd_altitude=var.altitude;
      //increase throttle slowly
      if(var.ctr_throttle<1.0)set_thr(var.ctr_throttle+apcfg.to_thrRise*dt);
      var.cmd_rpm=apcfg.rpm;
      if(var.rpm>=apcfg.rpm && apcfg.to_type==to_type_runway) stage++;
    }break;
    case TAKEOFF_stage::Run:{
      time_s=t;
      setNE(var.pos_NE);
    ...
    case TAKEOFF_stage::Liftoff_do:{
      ...
      if(var.altitude>=apcfg.to_liftAlt)stage++;
    }break;
    case TAKEOFF_stage::Climbing:{
      if(conf.ahrs_noMAG!=ahrs_noMAG_always) var.cmode&=~cmode_nomag;
    }break;
    case TAKEOFF_stage::Climbing_do:{
      var.calc();
      navigate();
      if(var.altitude>=apcfg.altitude*0.8) stage++;
    }break;
  }
  return stage;
}
//==============================================================================
uint Shiva::TAKEOFF_blimp(uint stage)
{
  switch(stage){
    case TAKEOFF_stage::init:{
    }break;
...
//==============================================================================
uint Shiva::TAKEOFF_boat(uint stage)
{
  switch(stage){
    case TAKEOFF_stage::init:{
    }break;
...
void Shiva::LANDING(void)
{
  if(prev_rwidx!=var.rwidx) { //protect runway change
    prev_rwidx=var.rwidx;
    if(var.stage) var.stage=LANDING_stage::Cancel;
  }
  uint stage=var.stage;
  switch(stage){
    case LANDING_stage::init:{
      ...
      var.ctr_airbrk=0;
      var.cmd_airspeed=apcfg.spd_cruise;
      if(apcfg.ld_AGL==ld_AGL_always)var.power|=power_agl;
      else var.power&=~power_agl;
      var.rwAdj=0;
      var.mtype=mtype_hdg;
      alt_corr=0;
    ...
    case LANDING_stage::Flare:{
      ...
      if(alt_speech>10)alt_speech=10;
      time_s=t;
      if(apcfg.ld_AGL)var.power|=power_agl;
      var.status&=~status_touch;
      stage++;
    }break;
    case LANDING_stage::Flare_do:{
      if(ers_landing)break;
      //say AGL altitude
      if((var.altitude>=0) && (var.altitude<alt_speech)) {
        dmsg("Landing: %u m to go\n",alt_speech);
        if(alt_speech>=10)alt_speech=7;
        else if(alt_speech>=7)alt_speech=5;
        else if(alt_speech>=5)alt_speech=3;
        else alt_speech--;
      }
      //check if touchdown
      if( (apcfg.ld_tdEvent==ld_tdEvent_altitude || (apcfg.ld_tdEvent==ld_tdEvent_agl && (!apcfg.ld_AGL))) ||
        ((apcfg.ld_tdEvent==ld_tdEvent_agl)&&(var.status&status_agl)) ) {
        if(var.altitude>apcfg.ld_tdAlt) time_s=t;
        else if((t-time_s)>200)stage=LANDING_stage::Touchdown;
      }
    }break;
  ...
  uint stage_s=stage;
  switch(apcfg.type){
    case type_airplane:         stage=LANDING_fixedwing(var.stage); break;
    case type_helicopter:       stage=LANDING_helicopter(var.stage); break;
    case type_blimp:            stage=LANDING_blimp(var.stage); break;
    case type_boat:             stage=LANDING_boat(var.stage); break;
  }
  if(var.stage==stage)var.stage=stage_s;
  else var.stage=stage;
}
//==============================================================================
uint Shiva::LANDING_fixedwing(uint stage)
{
  switch(stage){
    case LANDING_stage::init:{
      var.ctr_airbrk=0;
    ...
    case LANDING_stage::TA:{
      ...
      //set altitude
      _var_float v;
      if(apcfg.ldratioMin>0)v=apcfg.ldratioMin;
      else if(var.ldratio>0)v=var.ldratio;
      else v=10;
      v=(int)(2.0*(apcfg.turnR*(2.0*PI)+aLen)/v/app_Kwt)/10*10;
      if(ers_landing)v+=apcfg.ld_flareAlt;
      v=var.limit(v,apcfg.altitude,apcfg.altitude*3);
      if(var.cmd_altitude<v)var.cmd_altitude=v;
      //set airspeed
      ...
      if(ers_landing) dmsg("Landing: Parachute\n");
      sub_stage=0;
    }break;
    case LANDING_stage::TA_do:{
    ...
    case LANDING_stage::TAapp:{
      ...
      if(apcfg.spd_VNO>=apcfg.spd_cruise) var.cmd_airspeed=apcfg.spd_VNO;
    ...
    case LANDING_stage::TAapp_do:{
      ...
      if((t-time_stage_s)<3000)var.delta=0; //kf reset
      if(!adjust_landing_circle()){
        stage++;
      }
      if(apcfg.ld_autocancel && (!(var.error&error_rpm)) && (!(var.cmode&cmode_thrcut)) && (!(var.cmode&cmode_throvr))){
        if(var.delta>(-appLength))time_s=t;
        else if((t-time_s)>5000)stage=LANDING_stage::Cancel;  //cancel
      }
      ...
      if(apcfg.spd_VNO>apcfg.spd_cruise){
        ...
        v=var.limit(apcfg.spd_cruise+(apcfg.spd_VNO-apcfg.spd_cruise)/2.0+var.delta*apcfg.ld_speedKp,v,apcfg.spd_VNO);
        var.cmd_airspeed=v;
      }
      navigate();
    ...
    case LANDING_stage::App_do:{
      ...
      if(fabs(var.boundAngle(var.wpHDG-rwHDG))>90)dWP=-dWP;
      ...
      if(apcfg.ld_autocancel && (!(var.error&error_rpm)) && (!(var.cmode&cmode_thrcut)) && (!(var.cmode&cmode_throvr))){
        ...
        if(dWP>0){ //approaching
          if(var.delta<0){ //undershoot checks only
            bOk=dWP>(appLength*0.5) || var.delta>(var.cmd_airspeed*-7.0); //check delta on second half only
            bOk&=dWP>(appLength*0.3)|| fabs(var.rwDelta+var.rwAdj)<20; //check alignment
            bOk&=ers_landing || var.altitude>apcfg.ld_flareAlt;// || var.delta>(var.cmd_airspeed*-7.0);
          }
      ...
      if(apcfg.spd_VNO>apcfg.spd_cruise){
        //LD control
        _var_float v=apcfg.spd_cruise+(apcfg.spd_VNO-apcfg.spd_cruise)/2.0+var.delta*apcfg.ld_speedKp;
        //dist vs wind control
        if(app_Kwt<1.0)v-=var.delta/(app_Kwt)*apcfg.ld_windKp;
      ...
      if(ers_landing){
        ...
        if(dWP<0)stage++;
      }else if(var.altitude<=apcfg.ld_flareAlt) stage++;
      navigate();
      set_thr(apcfg.ld_thr);
      ctr_AirbrkApp();
      calcETA(vLE_total,var.airspeed);
    ...
    case LANDING_stage::Flare:{
      ...
      pitch_s=pitch_s2=pitch_s3=var.cmd_theta[1]; //to only increase pitch if below ld_finMinPitch
    ...
    case LANDING_stage::Flare_do:{
      ...
      if((!ctr_AirbrkDS(apcfg.ld_flareDS)) && var.ctr_airbrk>apcfg.ld_Airbrk_bias){
        ...
        if(var.ctr_airbrk<apcfg.ld_Airbrk_bias)var.ctr_airbrk=apcfg.ld_Airbrk_bias;
      }
      ctr_Course();
      _var_float delta=0;
      //-----------------------------
      if(ers_landing){
        if(!(var.ctrb&ctrb_ers)){
          ...
          ctr_Airspeed();
          if(apcfg.ld_flarePitch>0)set_pitch(apcfg.ld_flarePitch);
      ...
      //-----------------------------
        ...
        set_vspeed(-v,true);*/
        _var_float v=0;
        if(var.cmd_theta[1]>=apcfg.ld_flarePitch)v=-apcfg.ld_flareDS;
        set_vspeed(v,true);
        ctr_VSpeed();
        //pitch down limit, gradient pitch down protect
        if(var.cmd_theta[1]<apcfg.ld_flarePitch){
          if(var.cmd_theta[1]<pitch_s)set_pitch(pitch_s,false);
          else pitch_s=var.cmd_theta[1];
        }
      }else{
        ctr_Altitude();
        ...
        if(v<apcfg.spd_stall)v=apcfg.spd_stall;
        if(var.cmd_airspeed>v) var.cmd_airspeed=v; //only decrease
        ctr_Airspeed();
        ...
        if(apcfg.ld_flarePitch>0){// && pitch_s2<apcfg.ld_flarePitch){
          ...
          if(v<pitch_s3)v=pitch_s3;
          else pitch_s3=v; //growing limit only
        ...
        if(reg_Airspeed.out<0 || var.altitude<10){
          if(reg_Airspeed.out<pitch_s)set_pitch(pitch_s,false);
          else pitch_s=reg_Airspeed.out;
        }else pitch_s=reg_Airspeed.out;
      }
      ...
      delta=var.dWPT;
      if(fabs(var.boundAngle(var.wpHDG-rwHDG))>90)delta=-delta;
    ...
    case LANDING_stage::Touchdown:{
      ...
      if(pitch_s<0)pitch_s=0; //saved cmd_pitch
    ...
    case LANDING_stage::Touchdown_do:{
      if(var.status&status_agl)alt_corr=0;
      flytoLine(rwNE);
      ...
      if(ers_landing){
        ...
        if(fabs(var.vspeed)<3){
          if(apcfg.ld_runAz>=0 && var.acc.mag()>=(9.81+apcfg.ld_runAz) && (t-time_s)>200)
            stage++;
        }else time_s=t;
      }else if(apcfg.ld_flareDS>0 && apcfg.VSpeed.Lo>0){
        ...
        if(var.vcas>=0 || var.cmd_theta[1]<apcfg.ld_flarePitch)v=0;
        else if(var.vcas<0)v=var.limit(-var.vcas*v,0.0*apcfg.ld_flareDS,apcfg.ld_flareDS);
        if(var.airspeed<=apcfg.spd_stall)v=apcfg.ld_flareDS;
        set_vspeed(-v,true);
        ctr_VSpeed();
        //pitch down limit, gradient pitch down protect
        if(var.cmd_theta[1]<apcfg.ld_flarePitch){
    ...
    case LANDING_stage::Run_do:{
      ...
      if( ((t-time_s)>3000) || (var.gSpeed<=(apcfg.ld_runSpeed*0.9)) ) {
        if(var.ctr_brake<1.0) {
          ...
          if(var.ctr_brake>1.0)var.ctr_brake=1.0;
        }
      }
      //finished?
      if(var.gSpeed<(apcfg.ld_runSpeed*0.3))stage++;
    }break;
    case LANDING_stage::Run_done:{
      var.ctr_airbrk=0;
    }break;
    ...
    case LANDING_stage::Cancel_do:{
      ...
      var.cmd_course=var.tgHDG;
      navigate(false);
      if((!ers_landing) && var.cmd_theta[1]<pitch_s)set_pitch(pitch_s,false);
      stabilize();
      set_thr((apcfg.to_climbThr<=0)?1:apcfg.to_climbThr);
      if (var.altitude>(apcfg.altitude*0.8)) stage++;
    }break;
    ...
    case LANDING_stage::Finished:{
      var.ctr_brake=1;
      var.power&=~power_agl;
      if(ers_landing)var.ctrb|=ctrb_rel;
    }break;
    case LANDING_stage::Finished_do:{
      var.calc();
      ctr_Taxi();
...
//==============================================================================
uint Shiva::LANDING_helicopter(uint stage)
{
  switch(stage){
    case LANDING_stage::init:{
    ...
    case LANDING_stage::TAapp_do:{
      ...
      if(var.cmode&cmode_hover && var.dWPT<=apcfg.ld_maxRun && var.gSpeed<1 && fabs(var.boundAngle(var.theta[2]-var.cmd_theta[2]))<30)stage++;
    }break;
    case LANDING_stage::App:{
      var.cmd_airspeed=0;
      var.cmode|=cmode_hover;
      ...
      if(appLength<=0)var.cmd_altitude=apcfg.ld_flareAlt*0.5;
      pitch_s=var.cmd_altitude;
      if(apcfg.ld_AGL)var.power|=power_agl;
    }break;
    case LANDING_stage::App_do:{
      setNE(rwNE+var.rotate(Point(appLength*var.limit(var.altitude/apcfg.altitude,0,1),0),-(180.0+rwHDG)));
      ctr_HoverX();
      ctr_HoverY();
      set_yaw(var.cmd_course,false); //user input only
      if(appLength>0){
        var.cmd_altitude=apcfg.altitude*var.limit(var.distance(rwNE-var.pos_NE)/appLength,0,1)-1.0;
      }
      if(var.cmd_altitude>pitch_s)var.cmd_altitude=pitch_s;
      else pitch_s=var.cmd_altitude;
      ctr_Altitude();
      if(var.cmd_vspeed>-apcfg.ld_flareDS && var.dWPT<=apcfg.ld_maxRun && var.gSpeed<1)
        set_vspeed(-apcfg.ld_flareDS,false);
      else if(var.cmd_vspeed>0)set_vspeed(0,false);
      ctr_VSpeed();
      ctr_Governor();
      stabilize();
      if(var.altitude<=apcfg.ld_flareAlt) stage++;
    }break;
    case LANDING_stage::Flare:{
      var.cmode&=~cmode_hyaw;
      if(conf.ahrs_noMAG!=ahrs_noMAG_off) var.cmode|=cmode_nomag;
    }break;
    case LANDING_stage::Flare_do:{
      setNE(rwNE+var.rotate(Point(appLength*var.limit(var.altitude/apcfg.altitude,0,1),0),-(180.0+rwHDG)));
      ctr_HoverX();
      ...
      if(apcfg.ld_tdEvent==ld_tdEvent_altitude||(apcfg.ld_tdEvent==ld_tdEvent_agl && apcfg.ld_AGL==0)){
        if(!(var.ctr_collective<apcfg.to_liftPitch && var.vspeed>(-apcfg.ld_flareDS/2.0))) time_s=t;
        else if((t-time_s)>1000) stage++;
      }
    }break;
    case LANDING_stage::Flare_done:{
    ...
    case LANDING_stage::Touchdown:{
      if(var.ctr_collective>apcfg.to_liftPitch){
        if(apcfg.ld_brkSpeed<=0){
          ...
          if(var.ctr_collective<-1)var.ctr_collective=-1;
        }
      }
      var.ctr_brake=1;
    }break;
    ...
    case LANDING_stage::Cancel:{
      ...
      var.ctr_brake=0;
      var.power&=~power_agl;
      if(var.cmd_altitude<apcfg.altitude)var.cmd_altitude=apcfg.altitude;
    }break;
    case LANDING_stage::Cancel_do:{
      var.calc();
      ctr_HoverX();
...
//==============================================================================
uint Shiva::LANDING_blimp(uint stage)
{
  switch(stage){
    case LANDING_stage::init:{
    }break;
...
//==============================================================================
uint Shiva::LANDING_boat(uint stage)
{
  switch(stage){
    case LANDING_stage::init:{
    }break;
...
{
  if((t-time_VE)<107) return last_VE_result;
  time_VE=t;
  ...
  if(var.delta>0){
    //too close
    if(cSpan>180)cR+=adj;  ///(2.0*M_PI);
    else appLength+=PI*adj;
  }else if(var.delta<(-snap)){
    //too far, decrease circle
    if(appLength>aLen) appLength+=adj;
    else cR+=adj;
    if(cR<=apcfg.turnR){
      cR=apcfg.turnR;
      if(appLength>aLen){ //approach line was increased before
        if(cSpan<30) last_VE_result=false;
      }else{
        if(cSpan<80&&(var.delta<(-snap))) last_VE_result=false;
      }
    }
  }
  const _var_float cR_max=apcfg.turnR*5.0;
  if(cR>cR_max)cR=cR_max;
  if(cSpan<15 && var.delta<aLen)last_VE_result=false;
  var.turnR=rwLeft?-cR:cR;
...
void Shiva::get_landing_length(_var_float gnd_altitude)
{
  ...
  if(dCircle<=cR) dCircle=cR-dCircle; //inside circle
  else dCircle-=cR;
  //arc parameters
  const _var_float cHdg=var.heading(cuNE); //heading from UAV to circle center
  const _var_float arc_start=var.boundAngle360(cHdg+(rwLeft?90.0:-90.0)); //circle flight start course
  cSpan=var.boundAngle360(rwLeft?(arc_start-var.boundAngle360(rwHDG)):(var.boundAngle360(rwHDG)-arc_start));
  ...
  //var.user1=way;
  //arc way
  _var_float arc_way=var.wind_circle(arc_start,rwLeft?-fabs(cSpan):fabs(cSpan),cR);  //circle arc
  way+=arc_way;
