...
{
public:
  REG_PID(_ft_regPID *acfg) : sum(0),out(0),cfg(acfg) {}
  void reset(const _var_float v=0.0);
  _var_float step(_var_float err,_var_float vel,_var_float dt,_var_float hystKp=0);
...
{
public:
  REG_PI(_ft_regPI *acfg) : sum(0),out(0),cfg(acfg) {}
  void reset(const _var_float v=0.0);
...
{
public:
  REG_TPI(_ft_regPI *acfg) : sum(0),out(0),cfg(acfg) {}
  void reset(const _var_float v=0.0);
  _var_float step(_var_float err,_var_float dt);
  _var_float sum;
  _var_float out;
...
{
public:
  REG_P(_ft_regP *acfg) : out(0),cfg(acfg) {}
...
{
public:
  REG_PPI(_ft_regPPI *acfg) : sum(0),out(0),cfg(acfg) {}
  void reset(const _var_float v=0.0);
...
protected:
  ...
  bool flytoHover(const Point &ne,_var_float sdist=-1); //fly to NED straight, return true if reached and go to hover mode
  bool flytoLine(const Point &ne,_var_float sdist=-1); //fly to NED by line with rwHDG, return true if reached
  void flytoCircle(const Point &ne,_var_float r,bool bCCW=false,bool adj90=true); //fly around circle with radius r around NED
  ...
  void ctr_smooth(_var_float *v, _var_float vSet, _var_float speed);//slow down some controls (cmd_roll,cmd_pitch, etc)
