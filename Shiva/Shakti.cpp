...
bool Shakti::ctr_Roll(void)
{
    ...
    _var_float v = apcfg.dyn_rollReg == dyn_rollReg_PID ? reg_Roll.step(err, vel, dt)
                                                        : reg_Roll.stepPPI(err, vel, dt);
    if (apcfg.ar_Lp > 0) {
        v = var.limit(v
...
bool Shakti::ctr_Pitch(void)
{
    ...
    _var_float v = apcfg.dyn_pitchReg == dyn_pitchReg_PID ? reg_Pitch.step(err, vel, dt)
                                                          : reg_Pitch.stepPPI(err, vel, dt);
    if (apcfg.ap_Lp > 0) {
        v = var.limit(v
...
_var_float REG_PID::step(_var_float err, _var_float vel, _var_float dt, _var_float hystKp)
{
    ...
    if (isinf(vel) || isnan(vel))
        vel = 0;
    out = var.limit(cfg->Kp * var.hyst(err, hystKp), cfg->Lp) + var.limit(cfg->Kd * vel, cfg->Ld)
          + sum;
    if (cfg->Ki == 0)
        sum = 0;
    else
