...
void Shakti::init()
{
  ...
  if(var.ldratio<=0)var.ldratio=apcfg.ldratioMin;
  ...
  switch(apcfg.init_mode){
    case init_mode_TAXI:    var.mode=mode_TAXI; break;
    case init_mode_TAKEOFF: var.mode=mode_TAKEOFF; break;
    case init_mode_EMG:     var.mode=mode_EMG; break;
    default:
      var.mode=mode_TAXI;
  }
  if(apcfg.init_ignition==init_ignition_off) var.power&=~power_ignition;
  if(apcfg.init_servo==init_servo_off) var.power&=~power_servo;
  if(apcfg.init_thrcut==init_thrcut_off) var.cmode&=~cmode_thrcut;
  else if(apcfg.init_thrcut==init_thrcut_override) var.cmode|=cmode_throvr;

  if(apcfg.fuel>0)var.fuel=apcfg.fuel;
...
void Shakti::fly(_var_float a_dt)
{
  ...
  if(apcfg.dbrk_steering>0){
    if(var.ctr_brake>0){
      ...
      float err=(fabs(var.ctr_steering)-apcfg.dbrk_steering)/(1.0-apcfg.dbrk_steering);
  ...
  if(var.sb&sb_shutdown){
    if((t-time_shdn_s)>200)var.power=0;
  }else time_shdn_s=t;


  //fuel rate to capacity
  if(apcfg.frate>0){
    if(var.ctr_throttle>0) var.frate=apcfg.frate*var.ctr_throttle;
    else var.frate=0;
  }
  if(var.fuel>0 && var.frate>0 && (t-time_frate)>=500){//counter%(AHRS_FREQ/2)==0){
    time_frate=t;
    var.fuel-=var.frate/(3600.0*2.0);
    if(var.fuel<0)var.fuel=0;
  }
...
void Shakti::calc_stab(void)
{
  ...
  if(apcfg.stab_opt==stab_opt_HPF){
    if(fabs(turn_rate)<1)f_stab_turn.reset(turn_rate);
  ...
  //limit & filter
  vstab=fabs(vstab);
  if(apcfg.stab_opt==stab_opt_airspeed)vstab*=apcfg.spd_cruise-var.airspeed;
...
void Shakti::rc_updated(bool valid)
{
  if(valid) {
    ...
    rc_timeout=0;
    var.status&=~status_rc;
    if (var.mode<=mode_UAV) var.mode=mode_HOME;
  }
}
...
void Shakti::rpm_updated(bool valid)
{
  if(valid){
    ...
    if(apcfg.rpm_pflt && apcfg.rpm>0){
      //filter RPM peaks
      const uint ds=apcfg.rpm/(apcfg.rpm_pflt==rpm_pflt_p8?8:apcfg.rpm_pflt==rpm_pflt_p4?4:2);
      if(d>ds && rpm_cnt<5){
        rpm_cnt++;
        var.rpm=rpmv_s;
      }else{
  ...
  //rpm error
  if(rpm_time_s) var.error|=error_rpm;
}
//==============================================================================
void Shakti::safety()
{
  if(apcfg.frm_mode==frm_mode_wingC)return;
  ...
  if(t>5000){
    ...
    }else if(var.Ve>(apcfg.sf_Ve*hystV)) safety_Ve=false;
    //Voltage Vs
    if(!safety_Vs){
    ...
    }else if(var.Vs>(apcfg.sf_Vs*hystV)) safety_Vs=false;
    if(bVerr) var.error|=error_power;
  }

  if (var.mode!=mode_EMG && var.mode!=mode_TAKEOFF && var.mode!=mode_LANDING && var.mode!=mode_TAXI){
    //dHome
    ...
    //vspeedERS
    if((!(var.ctrb&ctrb_ers))&&(apcfg.sf_vspeedERS!=0)&&(var.vspeed<-apcfg.sf_vspeedERS)&&((apcfg.altitude<50)||(var.altitude<=(apcfg.altitude/2.0)))) {
      dmsg("Error: Variometer overspeed %i m/s\n",(int)var.vspeed);
      var.ctrb|=ctrb_ers;
    }
    if(bVerr && apcfg.sfVe_action){
      switch(apcfg.sfVe_action){
        case sfVe_action_landing: var.mode=mode_LANDING;break;
      }
    }
    //Voltage Ve ERS
    if(!safety_VeERS){
      if((apcfg.sf_VeERS!=0)&&(var.Ve>(apcfg.sf_VeERS*0.3))&&(var.Ve<=apcfg.sf_VeERS)) {
        dmsg("Voltage error: Ve %.1fv (<%.1fv)\n",var.Ve,apcfg.sf_VeERS);
        safety_VeERS=true;
        var.ctrb|=ctrb_ers;
      }
    }else if(var.Ve>(apcfg.sf_VeERS*hystV)) safety_VeERS=false;
  }
  ...
  if((t-time_wrn_s)>=1000){
    ...
    for(uint i=0;i<(sizeof(apcfg.wrn_bind)/sizeof(apcfg.wrn_bind[0]));i++){
      if(apcfg.wrn_bind[i]){
        ...
        if(v<apcfg.wrn_min[i])warn|=msk;
        else if(v>apcfg.wrn_max[i])warn|=msk;
        while((warn^wrn_mask)&msk&warn){ //changed low-high
          //print warning var descr
          uint16_t var_m=apcfg.wrn_bind[i];
          uint type;
          void *value_ptr;
          if(!var.get_ptr(var_m,&value_ptr,&type))break; //get var type
          const char *name;
          const char *descr;
          if(type==vt_enum)var_m|=0xFF00;
          if(!var.get_text_names(var_m,&name,&descr))break;
          dmsg("Warning: %s\n",descr);
          break;
        }
      }
...
void Shakti::checkRestrictedAreas(bool reset)
{
  ...
  switch(area.stage++){
    ...
    case 1:
      ...
      if(area.area && area.area->pointsCnt>=2) return;
      if(area.idx){
        ...
        area.stage=0;
      }else area.stage=1000; //wait mission update
      return;
    case 2: //find closest distance to all lines
      ...
      return;
    case 3:{
      Mission::_item_area_point *p=mission.area_point(area.area,area.pidx==area.area->pointsCnt?0:area.pidx);
      area.pidx++;
    ...
    case 5:{
      ...
      if( ((area.p1[1]>0) != (area.p2[1]>0)) &&
        (0 < (area.p2[0]-area.p1[0]) * (-area.p1[1]) / (area.p2[1]-area.p1[1]) + area.p1[0]) )
    ...
    case 1000:
      ...
      //fallthru
    case 1001: area.stage=1001; return;
    default:
      area.stage=0;
  }
}
...
void Shakti::starter()
{
  if(var.ctrb&ctrb_starter){
    var.ctrb&=~ctrb_starter;
    if(starter_stage==0)starter_stage=1;
  }
  if(starter_stage==0){
  ...
  switch(starter_stage){
    case 1: //check no RPM present
      var.sw&=~sw_starter;
      if(var.power&power_ignition) dmsg("Engine start procedure\n");
      else dmsg("Engine cold turn procedure\n");
      ...
      if(apcfg.st_choke && (var.power&power_ignition))var.ctr_mixture=1;
      var.ctrb|=ctrb_horn;
      time_starter_s=t;
      starter_stage++;
      //fallthru
    case 2:
      ...
      if((t-time_starter_s)<1000)return;
      time_starter_s=t;
      starter_stage++;
      //fallthru
    case 3:
      var.ctr_throttle=apcfg.st_thr;
      if((t-time_starter_s)<(apcfg.st_pause*1000))return;
      starter_stage++;
      //fallthru
    ...
    case 6: //warm-up
      ...
      return;
    case 7: //choke time
      if((t-time_starter_s)<(apcfg.st_choke*1000))return;
  ...
  var.sw&=~sw_starter;
  var.ctrb&=~ctrb_horn;
  if(apcfg.st_choke)var.ctr_mixture=0;
}
//==============================================================================
//==============================================================================
...
void Shakti::stabilize()
{
  switch(apcfg.type){
    case type_airplane:
      ctr_Roll();
      ctr_Pitch();
      if(!ctr_Slip()) set_rud(var.rc_yaw); // mix
...
void Shakti::navigate(bool do_stabilize)
{
  switch(apcfg.type){
    ...
    case type_helicopter:
      ...
      ctr_Altitude();
      ctr_VSpeed();
      if(!ctr_Governor())set_thr(var.rc_throttle);
      break;
    case type_blimp:
      ctr_Course();
      ctr_Altitude();
...
bool Shakti::flytoHover(const Point &ne,_var_float sdist)
{
  ...
  if(flyto(ne,sdist)&&fabs(var.boundAngle(var.course-var.cmd_course))<45){
    ...
    var.cmd_airspeed=0;
    return true;
  }else var.cmd_airspeed=var.limit(apcfg.HoverX.Lpp+(var.dWPT-sdist)*apcfg.heli_Khs,apcfg.HoverX.Lpp,var.cmd_airspeed);
  return false;
}
//==============================================================================
...
void Shakti::flytoCircle(const Point &ne,_var_float r,bool bCCW,bool adj90)
{
  ...
  //if(adj90){
    ...
    if(var.rwDelta>sdist) var.cmd_course=var.boundAngle(var.tgHDG-90);
    else if(var.rwDelta<-sdist) var.cmd_course=var.boundAngle(var.tgHDG+90);
  //}
}
//==============================================================================
bool Shakti::check_wp_reached(_var_float sdist)
{
  ...
  if((counter%(int)(0.2/dt))==0) {
    ...
    if(fabs(d)>=apcfg.wpt_drv)wpHDG_cnt++;
    else wpHDG_cnt=0;
...
void Shakti::checkModeChange(void)
{
  ...
  if(apcfg.heli_hYaw>0)var.cmode|=cmode_hyaw;
...
bool Shakti::goWpt(uint n)
{
  ...
  var.wpidx=n;
  const Point &wpNE=var.lla2ne(Vect(mission.current.wp->lat,mission.current.wp->lon,mission.current.wp->alt));
    ...
    for(int i=0;;i++){
      ...
      switch(a->hdr.option){
        case Mission::mo_speed:{ //fly with spec speed
          ...
          if(spd>apcfg.spd_VNO)spd=apcfg.spd_VNO;
          else if(spd<apcfg.spd_VSI)spd=apcfg.spd_VSI;
          set_airspeed(spd);
  ...
  if (var.mtype==mtype_line) {
    if (n==0)var.mtype=mtype_hdg;
    else{
      Mission::_item_wp *cwp=mission.current.wp; //preserve current wp
      Mission::_item_wp *pwp=mission.wp(n-1);
      mission.current.wp=cwp;
      if(!pwp)var.mtype=mtype_hdg;
      else var.tgHDG=var.heading(wpNE-var.lla2ne(Vect(pwp->lat,pwp->lon,pwp->alt)));
    }
  }
  ...
  for(int i=0;;i++){
    ...
    switch(a->hdr.option){
      case Mission::mo_shot:{ //make a cam shot
        ...
        switch(a->shot.opt){
          default:
          case 0: var.cams=cams_shot; break;
          case 1: var.cams=cams_dshot; var.cam_tperiod=a->shot.dist; dshot_ll.fill(); break;
          case 2: var.cams=cams_idle; break;
        }
      }break;
      case Mission::mo_loiter:{ //visit POI if any
        ...
        if(mission.current.pi)goPi(var.piidx);
        else{
          var.mode=mode_STBY;
          checkModeChange();
        ...
        //override limits
        if(a->loiter.turnR!=0)mission.current.turnR=a->loiter.turnR;
        if(a->loiter.loops>0)mission.current.loops=a->loiter.loops;
        if(a->loiter.timeS>0)mission.current.timeS=a->loiter.timeS;
...
{
  if(!mission.current.pi)return Vect();
  return Vect(Vect(mission.current.pi->lat,mission.current.pi->lon,mission.current.pi->hmsl==0?var.home_pos[2]:mission.current.pi->hmsl));
}
//==============================================================================
...
{
  //shooting controls
  if((t-time_shtr_s)>200) var.cam_ctrb&=~cam_ctrb_shtr;
  switch(var.cams){
    default:
    case cams_idle: break;
    case cams_shot:
  ...
  //gimbal controls
  if(!apcfg.cam_control)return;

  if(var.ctrb&ctrb_ers)var.power&=~power_payload;
  if(!(var.power&power_payload))var.cam_mode=cam_mode_camoff;
    ...
    switch(apcfg.cam_default){
      case cam_default_stab:
        ...
        break;
      case cam_default_position: var.cam_mode=cam_mode_position; break;
      case cam_default_speed: var.cam_mode=cam_mode_speed; break;
    }
  }
  ...
  switch(var.cam_mode){
    ...
    case cam_mode_position:
      //if(var.camcmd_theta[2]==0 && var.camcmd_theta[0]!=0)var.camcmd_theta[2]=var.camcmd_theta[0];
      var.cambias_theta=var.boundAngle(var.cambias_theta+var.camcmd_theta*dt*var.limit(1.0-var.cam_zoom,0.1,1));
    ...
    case cam_mode_target:{
      ...
      if((t-time_cam_s)>100){
        ...
        Point rc((var.camcmd_theta[0]+var.camcmd_theta[2])/180.0,var.camcmd_theta[1]/90.0);
        if(rc[0]>dw)adj[1]=rc[0]-dw;
        else if(rc[0]<-dw)adj[1]=rc[0]+dw;
        if(rc[1]>dw)adj[0]=-rc[1]+dw;
        else if(rc[1]<-dw)adj[0]=-rc[1]-dw;
        if(!adj.isNull()){
      ...
      if(apcfg.cam_control==cam_control_down){
        ...
        var.cambias_theta[1]=-var.heading(ne[0],var.distance(var.cam_tpos[2]-var.gps_pos[2],ne[1]));
      }
      doStab=true;
    }break;
  }
  switch(apcfg.cam_control){
    ...
    case cam_control_down:
      if(doStab)cam_stab_down();
      //var.cam_theta=Quat(var.theta*D2R).conjugate().qmult(var.camctr_theta*D2R).toEuler()*R2D;
      var.cam_theta=Quat(var.theta*D2R).qmult(var.camctr_theta*(180.0*D2R)).toEuler()*R2D;
      var.cam_theta[0]=0;
      var.camctr_theta+=Vect(apcfg.cam_align.x,apcfg.cam_align.y,apcfg.cam_align.z)/180.0;
...
void Shakti::cam_stab_front(void)
{
  ...
  qGimbal.fromEuler(Vect(var.cambias_theta[0]*D2R,var.cambias_theta[1]*D2R,0));//var.cambias_theta[0]*D2R),Quat::EulOrdXYXs);
  qUAV.conjugate();
  qGimbal.qmult(qUAV);
  ...
  if(euler[1]>-15){
    if(var.cambias_theta[0]>1)var.cambias_theta[0]-=(10.0+var.cambias_theta[0])*dt;
    else if(var.cambias_theta[0]<-1)var.cambias_theta[0]+=(10.0-var.cambias_theta[0])*dt;
    else var.cambias_theta[0]=0;
  }
...
void Shakti::cam_stab_down(void)
{
  ...
  var.camctr_theta=qUAV.conjugate().qmult(Vect(0,var.cambias_theta[1]*D2R,(var.cambias_theta[2])*D2R)).toEuler()*(R2D/180.0);
}
//==============================================================================
...
void Shakti::ctr_autoflaps()//_var_float cfgv,bool use_cmd)
{
  ...
  if(doAutoFlaps && apcfg.flaps_auto>0 && spdMin>0 && spdMax>spdMin){// && var.airspeed>apcfg.spd_stall
    //autoflaps or manual
    ...
    if(spd<spdMin) spd=spdMin;
    else if(spd>spdMax) spd=spdMax;
  ...
  //safe speeds
  if(apcfg.spd_VFE>apcfg.spd_flapsMax && var.airspeed>apcfg.spd_VFE) v=0;
  var.ctr_flaps=v;
}
//==============================================================================
//==============================================================================
...
{
  if(apcfg.Roll.Lo==0)return false;
  ...
  _var_float v=apcfg.dyn_rollReg==dyn_rollReg_PID?reg_Roll.step(err,vel,dt):reg_Roll.stepPPI(err,vel,dt);
  if(apcfg.ar_Lp>0){
    v=var.limit(v+var.limit(d_gyro[0].step(var.gyro[0],dt)/100.0*apcfg.ar_Kp,apcfg.ar_Lp),apcfg.Roll.Lo);
  }
...
{
  if(apcfg.Pitch.Lo==0)return false;
  ...
  const _var_float err=var.boundAngle(var.theta[1]-vcmd);
  const _var_float vel=var.gyro[1];
  _var_float Ks=get_gain(var.airspeed,apcfg.Ks_elevator_low,apcfg.Ks_elevator_high);
  _var_float v=apcfg.dyn_pitchReg==dyn_pitchReg_PID?reg_Pitch.step(err,vel,dt):reg_Pitch.stepPPI(err,vel,dt);
  if(apcfg.ap_Lp>0){
    v=var.limit(v+var.limit(d_gyro[1].step(var.gyro[1],dt)/100.0*apcfg.ap_Kp,apcfg.ap_Lp),apcfg.Pitch.Lo);
  }
  v=v*Ks/(-100.0);
...
{
  if(apcfg.Yaw.Lo==0)return false;
  switch(apcfg.type){
    case type_airplane:
    case type_helicopter:{
      ...
      if(apcfg.ay_Lp>0){
        v=var.limit(v+var.limit(d_gyro[2].step(var.gyro[2],dt)/100.0*apcfg.ay_Kp,apcfg.ay_Lp),apcfg.Yaw.Lo);
      }
      v/=-100.0;
    ...
    case type_blimp:{
      _var_float err=var.boundAngle(var.cmd_course-((var.cmode&cmode_hover)?var.theta[2]:var.course));
      _var_float vel=-var.gyro[2];
...
{
  if(apcfg.Thr.Lo==0)return false;
  _var_float err=0;
  switch(apcfg.type){
    case type_airplane:
  ...
  }else set_thr(v,false);
  return true;
}
//------------------------------------------------------------------------------
bool Shakti::ctr_Governor(void)
{
  if(apcfg.Governor.Lo==0 || (var.error&error_rpm))return false;
  ...
  if(apcfg.type==type_helicopter){
    if((var.ctrb&ctrb_gear) && apcfg.Thr.Lo!=0 && apcfg.RpmPitch.Lo!=0){
      ...
      _var_float err=((_var_float)var.rpm-(_var_float)var.cmd_rpm)/10000.0;//(_var_float)apcfg.rpm_max;
    ...
    if(var.cmd_rpm>target_rpm)var.cmd_rpm=target_rpm;
    else{
      uint rpm_step=(_var_float)apcfg.rpm_max*dt*apcfg.to_thrRise;
      if(!rpm_step)rpm_step=1;
      uint new_rpm=var.cmd_rpm+rpm_step;
      if(new_rpm<target_rpm)var.cmd_rpm=new_rpm;
      else var.cmd_rpm=target_rpm;
    }
  }else var.cmd_rpm=(_var_float)apcfg.rpm_idle+((_var_float)apcfg.rpm_max-(_var_float)apcfg.rpm_idle)*var.ctr_throttle;
  _var_float err=((_var_float)var.rpm-(_var_float)var.cmd_rpm)/10000.0;//(_var_float)apcfg.rpm_max;
...
{
  if(apcfg.ld_AirbrkDS.Lo==0)return false;
  _var_float err=var.vspeed+vDS;
  _var_float v=var.limit(reg_AirbrkDS.step(err,dt),0,100);
  uint iv=v/10;
  v=iv*10;
  v/=100.0;
  if(fabs(var.ctr_airbrk-v)>=0.1)var.ctr_airbrk=v;
  return true;
}
//------------------------------------------------------------------------------
bool Shakti::ctr_Thermal(void)
{
  if(apcfg.Thermal.Lo==0 || apcfg.type!=type_airplane)return false;
...
{
  if(apcfg.Course.Lo==0)return false;
  if(isnan(var.cmd_course)||isinf(var.cmd_course))var.cmd_course=0;
  switch(apcfg.type){
    case type_airplane:
    case type_helicopter:{
      ...
      if(apcfg.type!=type_helicopter && (var.mode==mode_UAV || var.mode==mode_WPT || var.mode==mode_HOME)){
        if(err>90 && var.cmd_theta[0]<(-tLo))err-=360;
        else if(err<(-90) && var.cmd_theta[0]>(tLo))err+=360;
      }
      if(apcfg.type==type_helicopter && apcfg.heli_topt && fabs(err)>90){
        if(apcfg.heli_rotor==heli_rotor_CW)err-=360;
        else err+=360;
      }
      //crs to roll reg PID/PPI
      _var_float vel=-var.gyro[2];
      _var_float v=(apcfg.dyn_courseReg==dyn_courseReg_PID)?reg_Course.step(err,vel,dt):reg_Course.stepPPI(err,vel,dt);
      _var_float Ks=get_gain(var.vXY[0]<0?0:var.vXY[0],apcfg.Ks_roll_gs_low==0?0:1.0/apcfg.Ks_roll_gs_low,0);
      v/=Ks;
      set_roll(v);
...
{
  if(apcfg.VSpeed.Lo==0)return false;
  switch(apcfg.type){
    case type_airplane: {
      ...
      if(apcfg.VSpeed.Kp==0){
        ...
        if(var.airspeed==0)err=0;
        else err/=(var.airspeed*var.airspeed);
    ...
    case type_helicopter:{
      if(apcfg.dyn_vspeedMin<0 && var.cmd_vspeed<apcfg.dyn_vspeedMin)var.cmd_vspeed=apcfg.dyn_vspeedMin;
      else if(apcfg.dyn_vspeedMax>0 && var.cmd_vspeed>apcfg.dyn_vspeedMax)var.cmd_vspeed=apcfg.dyn_vspeedMax;

      if(var.cmode&cmode_hover){
        if(apcfg.heli_vsMinH<0 && var.cmd_vspeed<apcfg.heli_vsMinH)var.cmd_vspeed=apcfg.heli_vsMinH;
        else if(apcfg.heli_vsMaxH>0 && var.cmd_vspeed>apcfg.heli_vsMaxH)var.cmd_vspeed=apcfg.heli_vsMaxH;
      }
    ...
    case type_blimp:{
      _var_float err=var.cmd_vspeed-var.vspeed;
      _var_float vel=var.acc[2]+9.81;   //Vect(var.acc-var.rotate(Vect(0,0,-9.81),var.theta))[2];
      _var_float v=reg_VSpeed.step(err,vel,dt);
      //control pitch
      v*=get_gain(var.airspeed,0,apcfg.Ks_pitch_high);
      set_pitch(v);
...
{
  if(apcfg.Altitude.Lo==0)return false;
...
{
  if(apcfg.Taxi.Lo==0)return false;
...
{
  if(apcfg.Brakes.Lo==0 || apcfg.taxi_gSpeed<=0)return false;
  if(apcfg.type!=type_airplane)return false;
...
{
  if(apcfg.Line.Lo==0)return false;
  _var_float err=var.rwDelta;
  _var_float vel=var.rwDV;
  _var_float adj=var.limit(var.boundAngle(((apcfg.Line.Ki==0)?reg_Line.stepPPI(err,vel,dt):reg_Line.step(err,vel,dt))),90);
  //_var_float cd=var.boundAngle(var.course-var.tgHDG);
  _var_float sdist=apcfg.turnR;
  if(var.rwDelta>sdist) adj=90;
  else if(var.rwDelta<-sdist) adj=-90;
...
{
  if(apcfg.Path.Lo==0)return false;
  _var_float err=var.rwDelta;
  _var_float vel=var.rwDV;
...
{
  if(apcfg.Airspeed.Lo==0)return false;
  smooth_cmd_airspeed();
  ...
  switch(apcfg.type){
    case type_airplane: {
      ...
      if(apcfg.spd_stall>0 && apcfg.spd_stall<apcfg.spd_cruise){
        _var_float vmin=apcfg.spd_stall*sqrt(1.0/cos(var.limit(var.cmd_theta[0],-80,80)*D2R));
        if(cmd<vmin)cmd=vmin;
      }
      ...
      if(apcfg.dyn_pitchDiff>0 && apcfg.dyn_pitchDiff<1.0){
        ...
        if(reg_Airspeed.sum<vmin) reg_Airspeed.sum=vmin;
      }
      reg_Airspeed.out=v;
      set_pitch(v);
    }break;
...
{
  if(apcfg.HoverY.Lo==0)return false;
  ...
  if((var.cmode&cmode_hyaw) && apcfg.heli_hYaw>0 && fabs(var.cmd_theta[0])>apcfg.heli_hYaw){
    if(var.cmd_theta[0]>0)var.cmd_course+=5.0*dt;
    else var.cmd_course-=5.0*dt;
...
_var_float Shakti::get_gain(_var_float speed,_var_float Ks_low,_var_float Ks_high)
{
  ...
  if(speed<=cs) {
    ...
    if(Ks_low<=1.0 || ls>=cs)return 1.0;
    if(speed<=ls)return Ks_low;
    _var_float x=1.0-(speed-ls)/(cs-ls);
    return x*x*(Ks_low-1.0)+1.0;
  }
  _var_float hs=apcfg.Ks_speed_high;
  if(Ks_high==0 || Ks_high>1.0 || hs<=cs) return 1.0;
  if(speed>=hs)return Ks_high;
...
_var_float Shakti::get_gain_rpm(_var_float Ks_low)
{
  ...
  if(Ks_low<=1.0 || ls>=cs)return 1;
  if(rpm<=ls)return Ks_low;
...
{
  if(var.mode==mode_TAKEOFF || var.mode==mode_TAXI || (var.mode==mode_LANDING && var.stage>2))return 1.0;
  if(Ks_slw==0 || Ks_slw>1 || Ks_slw<-1)return 1.0;
...
void Shakti::set_elv(_var_float v,bool doMix)
{
  if (doMix)
    v+=var.limit(get_gain(var.airspeed,0,apcfg.Ks_elevator_high)
       *fabs(var.cmd_theta[0])*apcfg.mix_elv_Kp,apcfg.mix_elv_Lo)/100.0; //mix to roll
...
void Shakti::set_rud(_var_float v,bool doMix)
{
  if(apcfg.type==type_helicopter){
    if (doMix){
      ...
      v+=var.limit(Ks*fabs(var.ctr_collective)*apcfg.mix_crud_Kp,apcfg.mix_crud_Lo)/100.0; //mix to collective
    }
    v=var.limit(v,1.0);
    var.ctr_rudder=v;
    if(apcfg.heli_yesc){
      ...
      if(!bInv){
        r0=-r0;
        if(reg_Yaw.sum>r0)reg_Yaw.sum=r0;
      }else{
        if(reg_Yaw.sum<r0)reg_Yaw.sum=r0;
      }
    }
    ...
    if (doMix){
      ...
      v+=var.limit(Ks*var.cmd_theta[0]*apcfg.mix_rud_Kp,apcfg.mix_rud_Lo)/100.0; //mix to roll
      v+=var.limit(Ks*var.gyro[0]*apcfg.mix_grud_Kp,apcfg.mix_grud_Lo)/100.0; //mix to roll gyro
    }
...
{
  if(apcfg.type==type_airplane || (apcfg.type==type_helicopter && var.mode!=mode_STBY && var.mode!=mode_UAV))
    v+=var.rc_roll*20.0;   //correct roll from joystick
  if(!smooth)spd=0;
  else if(spd==0) spd=apcfg.dyn_roll;
...
void Shakti::set_pitch(_var_float v,bool smooth,_var_float spd)
{
  ...
  if(!smooth)spd=0;
  else if(spd==0) spd=apcfg.dyn_pitch;
...
{
  if(!smooth)spd=0;
  else if(spd==0) spd=apcfg.dyn_yaw;
...
{
  if(!smooth)spd=0;
  else if(spd==0) spd=apcfg.dyn_vspeed;
...
{
  var.cmd_airspeed=v;
  if(var.cmd_airspeed<apcfg.spd_stall)var.cmd_airspeed=apcfg.spd_stall;
  else if(apcfg.spd_VSI>apcfg.spd_stall && var.ctr_flaps==0 && var.cmd_airspeed<apcfg.spd_VSI)var.cmd_airspeed=apcfg.spd_VSI;
  else if(apcfg.spd_VNE>apcfg.spd_stall && var.cmd_airspeed>apcfg.spd_VNE)var.cmd_airspeed=apcfg.spd_VNE;
}
//-----------------------------
...
{
  if(isinf(vSet) || isnan(vSet))vSet=0;
...
void Shakti::smooth_cmd_airspeed()
{
  ...
  if(apcfg.dyn_speed>0){
    if(cmd_airspeed_dyn>v) ctr_smooth(&cmd_airspeed_dyn,v,apcfg.dyn_speed*(100.0/2.0));
    else cmd_airspeed_dyn=v;
  }else cmd_airspeed_dyn=v;
}
//==============================================================================
//==============================================================================
void Shakti::print_status_flag(uint8_t var_idx,_var_flag value_prev)
{
  uint type;
  void *value_ptr;
  if(!var.get_ptr(var_idx,&value_ptr,&type))return;
  ...
  while(smask) {
    if(change&smask){
      const char *name;
      const char *descr;
      if(!var.get_text_names(var_idx|(smask<<8),&name,&descr))break;
...
void Shakti::print_status_enum(uint8_t var_idx,_var_enum value_prev)
{
  ...
  const char *name;
  const char *descr;
  if(!var.get_text_names(var_idx|0xFF00,&var_name,&var_descr))return;
  if(!var.get_text_names(var_idx|(v<<8),&name,&descr))return;
  dmsg("%s: %s\n",var_descr,descr);
}
//==============================================================================
void Shakti::print_status_float(uint8_t var_idx,_var_float *value_prev)
{
  ...
  *value_prev=v;
  const char *name;
  const char *descr;
  if(!var.get_text_names(var_idx,&name,&descr))return;
  const char *str=descr;
...
{
  if(apcfg.wpt_snap==0)apcfg.wpt_snap=15;
  if(apcfg.turnR==0)apcfg.turnR=300;

  switch(apcfg.type){
    case type_airplane:
      if(apcfg.to_liftAlt==0)apcfg.to_liftAlt=10;
      if(apcfg.to_liftPitch==0)apcfg.to_liftPitch=10;
      if(apcfg.spd_stall==0)apcfg.spd_stall=15;
      ...
      if(apcfg.spd_VY<=apcfg.spd_stall)apcfg.spd_VY=apcfg.spd_stall+1;
      if(apcfg.spd_cruise==0)apcfg.spd_cruise=apcfg.spd_stall+3;
      break;
    case type_helicopter:
      if(apcfg.spd_cruise==0)apcfg.spd_cruise=5;
      break;
    case type_blimp:
      if(apcfg.spd_cruise==0)apcfg.spd_cruise=5;
      break;
...
//==============================================================================
//==============================================================================
_var_float REG_PID::step(_var_float err,_var_float vel,_var_float dt,_var_float hystKp)
{
  if(isinf(err) || isnan(err))err=0;
  if(isinf(vel) || isnan(vel))vel=0;
  out=var.limit(cfg->Kp*var.hyst(err,hystKp),cfg->Lp)+var.limit(cfg->Kd*vel,cfg->Ld)+sum;
  if(cfg->Ki==0)sum=0;
  else sum=var.limit(sum+err*cfg->Ki*dt,cfg->Li);
...
_var_float REG_PID::stepPPI(_var_float err,_var_float vel,_var_float dt)
{
  ...
  if(isinf(vel) || isnan(vel))vel=0;
  err=err*cfg->Kp;
  if(cfg->Lp!=255)err=var.limit(err,cfg->Lp);
  vel+=err;
  out=var.limit(cfg->Kd*vel,cfg->Ld)+sum;
  if(cfg->Ki==0)sum=0;
  else sum=var.limit(sum+vel*cfg->Ki*dt,cfg->Li);
  out=var.limit(out,cfg->Lo);
  return out;
}
...
_var_float REG_TPI::step(_var_float err,_var_float dt)
{
  if(isinf(err) || isnan(err))err=0;
  if(cfg->Ki==0)sum=0;
  else sum=var.limit(sum+var.limit(err*cfg->Ki*dt,cfg->Li),100.0-cfg->Lo,100.0);
...
_var_float REG_PPI::step(_var_float err,_var_float vel,_var_float dt)
{
  ...
  if(isinf(vel) || isnan(vel))vel=0;
  err=err*cfg->Kpp;
  if(cfg->Lpp!=255)err=var.limit(err,cfg->Lpp);
  vel+=err;
...
{
  if(cfg->Ki==0)sum=0;
  else sum=var.limit(v,100.0-cfg->Lo,100.0);
  out=v;
}
