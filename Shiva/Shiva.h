...
private:
    ...
    uint32_t TAKEOFF_fixedwing(uint32_t stage);
    uint32_t TAKEOFF_helicopter(uint32_t stage);
    uint32_t TAKEOFF_blimp(uint32_t stage);
    uint32_t TAKEOFF_boat(uint32_t stage);

    uint32_t LANDING_fixedwing(uint32_t stage);
    uint32_t LANDING_helicopter(uint32_t stage);
    uint32_t LANDING_blimp(uint32_t stage);
    uint32_t LANDING_boat(uint32_t stage);
    ...
    uint32_t time_s;     // general stage timeout
    int alt_speech;      // landing say altitude
    union {
        uint32_t time_thr_s;   // takeoff throttle delay
        uint32_t time_stage_s; // takeoff throttle delay
        uint32_t sub_stage;    // sub stage used on landing
    };
    //taxi
    bool setup_taxiway();
    ...
    uint32_t turnsVE;     // number of circles
    uint32_t time_VE;
    bool last_VE_result;
