...
: silent(false),name(name),err_mute(false),init_stage(0),tcpdebug(false)
{
...
bool _tcp_client::connect_task()
{
  ...
  switch(init_stage){
    ...
    case 1:{ //connecting
      ...
      if(status==0){
        if(tcpdebug)printf("[%s]Connected immediately.\n",name);
        init_stage+=2;
        err_mute=false;
        break;
      }
      if(errno==0 || errno==EINPROGRESS){
        if(tcpdebug)printf("[%s]Connecting...\n",name);
        err_mute=false;
        init_stage++;
        break;
      }
      if(!err_mute)printf("[%s]Error: TCP Connect Failed (%i).\n",name,errno);
      err_mute=true;
      err="connect";
    }break;
    case 2:{ //wait socket connected
      ...
      tv.tv_sec = 0;
      tv.tv_usec = 0;
      int rv=select(fd+1,NULL,&write_fds,NULL,&tv);
      if(rv<=0)break;
      int result;
    ...
    case 3:{ //send request
      if(host.path[0]==0){
        if(tcpdebug)printf("[%s]Direct pipe.\n",name);
        init_stage=100;
        return true;
      }
    ...
    case 5: //header
      ...
      if(strstr(line_buf,"Server: ")){
        ...
        if(strchr(host.server,'.'))strchr(host.server,'.')[0]=0;
        break;
      }
      if(strlen(line_buf)==0){
        if(host.stream){
          if(!silent)printf("[%s]Connected (%s)\n",name,host.server);
          init_stage=100;
          return true;
        }
        err="header";
