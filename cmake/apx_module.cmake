...
# cmake-format: on
    ...
    if(GENSRC)
        foreach(gensrc ${GENSRC})
            get_target_property(gensrc_targets ${gensrc}_lib INTERFACE_SOURCES)
            list(APPEND srcs_check ${gensrc_targets})
        endforeach()
    endif()
    foreach(src ${srcs_check})
    ...
    if(has_sources)
        ...
        target_sources(${MODULE} INTERFACE ${SRCS})
    endif()
    ...
    if(GENSRC)
        foreach(gensrc ${GENSRC})
            if(has_sources)
                target_link_libraries(${MODULE} PUBLIC ${gensrc}_lib)
            else()
                target_link_libraries(${MODULE} INTERFACE ${gensrc}_lib)
            endif()
            # add_dependencies(${MODULE} ${gensrc}_lib)
            # if(has_sources)
            #     target_include_directories(${MODULE} PUBLIC ${gensrc_dir})
            #     target_sources(${MODULE} PRIVATE $<TARGET_OBJECTS:${gensrc}_lib>)
            # else()
