...
//=============================================================================
#ifdef FLT_CHECK_NAN
#define FLT_CHECK_NAN_IMPL(avalue) if(!matrixmath::f_isvalid(avalue)){avalue=0;reset();}
#else
...
public:
  Derivative()
  : index_0(0),index_1(1),index_3(3),index_4(4),init_cnt(0)
  {
    reset();
  }
...
public:
  ...
  float moving_average(const float &value)
  {
    ...
    float average_inc=0;
    for(uint i=0;i<window_size;i++)average_inc+=incs[i];
  ...
  void reset(const float &v=0)
  {
    for(uint i=0;i<window_size;i++)incs[i]=v;
    //average_inc=v;
  }
private:
...
//=============================================================================
template <uint window_size=1>
class DynamicWindow
{
public:
  DynamicWindow()
  : index(0),middle((window_size + 1) / 2)
  {
    reset();
  }
  void reset(const float &v=0)
  {
    for(uint i=0;i<window_size;i++)data[i]=v;
  }
...
public:
  DerivativeX()
  : index_l(0),index_h(window_size/2),index(0),init_cnt(0),diff(0)
  {
    reset();
  }
  float step(float value,const float &dt)
  {
    return step(value)/dt;
  }
  ...
  void reset(const float &v=0)
  {
    for(uint i=0;i<window_size;i++)f_dv[i]=v;
    diff=0;
    init_cnt=0;
  }
private:
